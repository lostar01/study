...
type NginxStats struct {
	...
	// Connection (Reading - Writing - Waiting)
...
func ScanBasicStats(r io.Reader) ([]NginxStats, error) {
	...
	for s.Scan() {
		...
		if fields[0] == "Reading:" {
			...
			readingConns := Connections{Type: "reading", Total: r}
			writingConns := Connections{Type: "writing", Total: w}
			waitingConns := Connections{Type: "waiting", Total: wa}
